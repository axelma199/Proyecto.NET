    ...
    public partial class FormProducto : Form
    {
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                int codigoCategoria=conexion.obtenerCategoriaPorDescripcion(this.categoría.Text);
                Producto c = new Producto(int.Parse(this.codigo.Text), (this.descripción.Text), decimal.Parse(this.precio.Text), int.Parse(this.cantidad.Text), new CategoriaProducto(codigoCategoria,""));

                bool opc = conexion.insertar(c); //Inserta producto en base de datos.
                if (opc)
                {
                ...
                {
                    string message = "Error, código de producto ya existe en base de datos o código de categoría asociada no existe, intente de nuevo.";
                    string title = "Mensaje";
                    MessageBox.Show(message, title);
                }
            }
        ...
        {
            if (nombreProdAbast.Text != "" && cantidadProdAbast.Text != "")
            {
                try
                {
                    ProductoBD prod = new ProductoBD();
                    prod.abastecerInventarioProducto((nombreProdAbast.Text), int.Parse(cantidadProdAbast.Text));
            ...
            {
                string message = "Error, debe rellenar los campos respectivo.";
                string title = "Mensaje";
                MessageBox.Show(message, title);
            }
        }
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            ProductoBD conexion = new ProductoBD();
            lista = conexion.listarProductos();
